...
2. use `jupyter` to create a python script from the desired notebook:
`(my_env) $ jupyter nbconvert --to python my_notebook.ipynb`

3. run the notebook (in our environment) with `nohup` so that the shell can be disconnected and `&` to run the command in the background 
`(my_env) $ nohup python ./my_notebook &`
