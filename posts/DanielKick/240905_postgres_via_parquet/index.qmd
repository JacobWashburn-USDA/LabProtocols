...
find and replace kickd -\> loremipsum
...
``` sql
postgres=# CREATE ROLE kickd LOGIN PASSWORD 'TotallyRealPassword';
CREATE ROLE
postgres=# ALTER ROLE kickd CREATEDB;
ALTER ROLE
postgres=# GRANT ALL PRIVILEGES ON DATABASE apsimxsim TO kickd;
GRANT
apsimxsim=# GRANT ALL ON SCHEMA public TO kickd;
GRANT
```
...
We're not done with setup yet. We also need to change the search path for the kickd user so we can *find* our tables.
